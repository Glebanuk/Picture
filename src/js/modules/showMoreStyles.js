...
const showMoreStylesjs = (triger, wrapper) =>  {
  const btn = document.querySelector(triger);
    ...
    btn.addEventListener('click', function()  {
      getResourse('http://localhost:3000/styles') //отправляем запрос на сервер и он возвращает промис который нужно обработать. НО можно получать доступ напрямую если указать путь к файлу , (assets/db.json)
        .then(res => createCards(res)) // промис возвращает массив который передается в функцию написанную ниже.Если использовать доступ на прямую через assets/db.json то нужно указать res.styles
